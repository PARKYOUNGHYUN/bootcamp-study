...
#Train shape : (312, 864)
#Val   shape : (79, 864)
#앙상블 모델 1개 학습 완료 (random_states=[42])
#테스트 이미지 수 : 110
#정상 수          : 40
#이상 수          : 70
#validation 앙상블 점수 일부: [ 0.0206  0.0349  0.0242 -0.0056  0.0242  0.0346  0.0309  0.0221  0.0259
#  0.0411]
#Threshold (val 하위 65%, 중앙값) : 0.0343
#Confusion Matrix
#[[ 3 37]
# [ 4 66]]
#
...
#
#      normal       0.43      0.07      0.13        40
#     anomaly       0.64      0.94      0.76        70
#
#    accuracy                           0.63       110
#   macro avg       0.53      0.51      0.45       110
#weighted avg       0.56      0.63      0.53       110
#
#Anomaly F1-score : 0.7630
#ROC AUC (참고용) : 0.5368
#검출된 박스 수: 1
...
# %%
RANDOM_STATES = [42]   # 앙상블에 사용할 random_state 목록

models = []
for rs in RANDOM_STATES:
    # Isolation Forest 모델 생성
    m = IsolationForest(
        n_estimators=500,   # 트리 개수 (500)
        contamination=0.1, # 이상 비율을 대략 10%로 가정
       max_features=0.8,       # 전체 특징 중 80%만 랜덤 사용 (과적합 방지를 위해 추가)
        random_state=rs     # 결과 재현용 고정값
    )
    m.fit(X_tr)
    models.append(m)
