#%% [markdown]
#정상 학습 이미지 수   : 391
#원본 특징 벡터 shape : (391, 864)
#Train  : 312장
#Val    : 79장
#Train shape : (312, 864)
#Val   shape : (79, 864)
#앙상블 모델 3개 학습 완료 (random_states=[42, 123, 7])
#테스트 이미지 수 : 110
#정상 수          : 40
#이상 수          : 70
#validation 앙상블 점수 일부: [0.0614 0.0751 0.0657 0.0252 0.0592 0.0786 0.0666 0.0591 0.0677 0.0756]
#Threshold (val 하위 65%, 중앙값) : 0.0723
#Confusion Matrix
#[[ 5 35]
# [ 5 65]]
#
...
#
#      normal       0.50      0.12      0.20        40
#     anomaly       0.65      0.93      0.76        70
#
#    accuracy                           0.64       110
#   macro avg       0.57      0.53      0.48       110
#weighted avg       0.60      0.64      0.56       110
#
#Anomaly F1-score : 0.7647
#ROC AUC (참고용) : 0.5725
#검출된 박스 수: 1
#박스 (x, y, w, h, area): (34, 30, 188, 197, 28459.5)
...
# CLAHE 객체 전역 1회 생성
# → build_mean_reference, localize_anomaly 두 곳에서 공유
...
for fname in sorted(os.listdir(train_good_dir)):
    ...
    feat, _, _ = extract_lbp_feature(fpath)
    if feat is not None:
        X_train.append(feat)
        train_paths.append(fpath)

X_train = np.array(X_train)
print(f"정상 학습 이미지 수   : {len(X_train)}")
print(f"원본 특징 벡터 shape : {X_train.shape}")
...
# StandardScaler는 오히려 빈도 낮은 이상치 bin을 과도하게 증폭시킬 수 있으므로
# 정규화 없이 원본 특징 벡터를 그대로 사용합니다.
...
# %% [markdown]
# ## 8. Isolation Forest 앙상블 학습 (Ensemble Voting)
#
# ### 왜 앙상블인가?
...
def ensemble_score(X_input):
    """앙상블 모델 각각의 decision_function 점수를 평균합니다."""
...
# %% [markdown]
# ## 11. Threshold 설정 — validation 점수 하위 65% (중앙값)
#
# - 모델은 train(80%)으로만 학습
