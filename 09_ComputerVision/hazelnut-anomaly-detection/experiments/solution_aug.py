...
# | Threshold | train 하위 5% 고정 | validation 하위 65% (실험적 최적값) |
...
def extract_lbp_feature(image_path, size=IMG_SIZE):
    """
...
# %% [markdown]
# ## 5-1. 데이터 증강 — 회전 이미지 추가
#
# ### 왜 회전 증강인가?
# 헤이즐넛이 살짝 돌아간 정상 이미지를 학습한 적 없으면
# 회전된 정상 이미지를 이상으로 오탐할 수 있습니다.
# 30° 간격으로 회전한 이미지를 정상 샘플로 추가하면:
# - 정상 패턴의 범위가 넓어져 오탐이 감소합니다.
# - 학습 데이터가 늘어나 모델 안정성이 향상됩니다.
#
# 구현이 단순합니다: 복잡한 윤곽 추출 없이 이미지 전체를 회전합니다.

# %%
AUGMENT_ANGLES = range(30, 360, 30)   # 30°, 60°, ..., 330° (11개 각도)

h_aug, w_aug = IMG_SIZE[1], IMG_SIZE[0]
center = (w_aug // 2, h_aug // 2)

def _extract_lbp_from_array(img_bgr):
    """이미 읽힌 BGR 배열에서 직접 LBP 특징을 추출합니다 (증강용)."""
...
for fpath in train_paths:
    ...
    for angle in AUGMENT_ANGLES:
        ...
        rotated = cv2.warpAffine(img, M, (w_aug, h_aug),
                                 flags=cv2.INTER_LINEAR,
                                 borderMode=cv2.BORDER_CONSTANT,
                                 borderValue=0)   # 헤이즐넛 배경(검정)과 일치시켜 가짜 LBP 패턴 방지
        aug_features.append(_extract_lbp_from_array(rotated))

aug_features = np.array(aug_features)

# 원본 + 증강 합치기
X_train_raw = np.vstack([X_train_raw, aug_features])
print(f"증강 후 학습 데이터 수 : {len(X_train_raw)} "
      f"(원본 {len(train_paths)}장 + 증강 {len(aug_features)}장)")
...
# %% [markdown]
# ## 12. Threshold 설정 — validation 점수 하위 65%
#
# - 모델은 train(80%)으로만 학습
# - Threshold는 **validation(20%) 점수의 하위 65%** 로 결정
#   - 실험 결과 65%ile에서 Anomaly F1=0.765, Recall=0.93 으로 최고 성능
#   - 70%ile 이상부터 F1이 내려가고 Normal Recall이 0.07 이하로 급락
#   - 테스트 레이블(`y_true`)은 이 과정에 전혀 개입하지 않음

# %%
threshold = np.percentile(val_scores, 65)
print(f"Threshold (val 하위 65%) : {threshold:.4f}")
...
plt.ylabel('빈도')
plt.title('Validation 점수 분포 및 Threshold (하위 65%)')
plt.legend()
plt.tight_layout()
plt.show()
...
# %% [markdown]
# ## 14. 오탐(False Positive) 분석
#
# 모델이 **정상인데 이상으로 판단한 이미지**를 직접 확인합니다.
# 오탐 원인(먼지, 회전, 조명 차이 등)을 파악하면
# 파라미터 튜닝보다 실질적인 개선 방향을 찾을 수 있습니다.

# %%
# FP: y_true==0(정상) 이고 y_pred==1(이상 판정)인 샘플
fp_indices = [i for i in range(len(y_true)) if y_true[i] == 0 and y_pred[i] == 1]
print(f"오탐(FP) 수 : {len(fp_indices)}장")

n_fp = min(len(fp_indices), 20)   # 최대 20장 표시
cols = 5
rows = (n_fp + cols - 1) // cols

plt.figure(figsize=(cols * 3, rows * 3))
for i, idx in enumerate(fp_indices[:n_fp]):
    img = cv2.imread(test_paths[idx])
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    plt.subplot(rows, cols, i + 1)
    plt.imshow(img)
    plt.title(
        f"{defect_types[idx]}\nScore:{test_scores[idx]:.3f}",
        fontsize=8, color='red'
    )
    plt.axis("off")

plt.suptitle(
    f"오탐(FP) {len(fp_indices)}장 — 정상인데 이상으로 판정\n"
    "공통 패턴(먼지, 회전, 조명 차이 등)을 육안으로 확인하세요",
    fontsize=11
)
plt.tight_layout()
plt.show()
